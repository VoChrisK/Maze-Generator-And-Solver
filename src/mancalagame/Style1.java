...
/**
 * This class is an extension of the BoardShape class. This class draws the
 * pits, mancala, and stones in a different style.
 * @author Chris Vo, Kevin Prakasa, Dung Pham
 * @Version 1.0
 */
public class Style1 extends BoardShape {
	//class variable declarations
	private Color c;
	/**
	 * construtor which extends from boardshape
	 * set color for each board, stones and pits
	 * @param height
	 * @param width
	 * @param model
	 * @param c
	 */
	public Style1(int height, int width, MancalaModel model, Color c)
	{
		super(height, width, model);
		this.c = c;
	}
	/**
	 * get color for each stones
	 * @return
	 */
...
/**
 * draw the stones and set color and shape for each stones
 * as well as the board and pit
 * @param g2
 * @param mancala1
 * @param mancala2
 */
	private void drawStones(Graphics2D g2, RoundRectangle2D mancala1, RoundRectangle2D mancala2) {
		...
		for(int i= 0 ; i< 6 ; i++){
			...
			for(int j = 0 ; j < super.getModel().getPlayers()[1].getPit()[i].getStone(); j++) {
				XAXIS = super.getEllipses()[1][i].getMinX() + (20 * counter);
				if(!(super.getEllipses()[1][i].contains(XAXIS, YAXIS))) {
					counter = 1;
					YAXIS += 20;
					XAXIS = super.getEllipses()[1][i].getMinX() + (20 * counter);
				}
				temp = new Rectangle2D.Double(XAXIS, YAXIS, 15, 15);
				g2.fill(temp);
				g2.draw(temp);
				counter++;
			}
		...
		for(int i= 0 ; i< 6 ; i++){
			...
			for(int j = 0 ; j < super.getModel().getPlayers()[0].getPit()[i].getStone(); j++) {
				XAXIS = super.getEllipses()[0][i].getMinX() + (20 * counter);
